...
def _get_selective_mcp_tools(declared_tool_names: list[str]) -> tuple[list, "MCPClient"]:
    ...
    log.info(
        ...
        declared_tool_names, available_names, [t.tool_name for t in selected],
    )
...
def _build_static_tools() -> None:
    ...
    def search_and_load_skill(query: str) -> str:
        ...
        log.info("Registry search: %d AGENT_SKILLS result(s) for '%s'", len(skill_records), query)
            ...
            try:
                ...
                if skill_records:
                    ...
                    log.info(
                        ...
                        skill_name_found, declared_tool_names,
                    )
                ...
                if _discovered_mcp_url and declared_tool_names:
                    try:
                        ...
                        _step(
                            "MCP connected — loaded " + str(len(mcp_tools))
                            + "/" + str(len(declared_tool_names))
                            + " declared tools: " + ", ".join(t.tool_name for t in mcp_tools),
                            "🔌"
                        )
